...
if [[ -n "$1" ]]; then
    if [[ ! -e "$1" ]]; then
        ...
        error_exit "File is not readable: $1"
    elif [[ ! -s "$1" ]]; then
        error_exit "File is empty: $1"
    else
        nl "$1"
    fi
else
    # Kein Datei-Argument vorhanden -> Benutze STDIN
    # Wenn STDIN leer ist, gibt nl nichts aus, das ist in Ordnung.
    # Wenn STDIN nicht leer ist, nummeriere die Zeilen.
    nl
fi
```
...
if [ "$now" -gt $(date -d "7:45:00" +%s) ] && [ "$now" -lt $(date -d "8:40:00" +%s) ]; then
    Fach="NW2-p"
fi

if [ "$now" -gt $(date -d "8:40:00" +%s) ] && [ "$now" -lt $(date -d "9:35:00" +%s) ]; then
    Fach="AM"
fi

if [ "$now" -gt $(date -d "9:35:00" +%s) ] && [ "$now" -lt $(date -d "10:30:00" +%s) ]; then
    Fach="SYTE"
fi

if [ "$now" -gt $(date -d "10:30:00" +%s) ] && [ "$now" -lt $(date -d "11:35:00" +%s) ]; then
    Fach="ITP2PM"
fi

if [ "$now" -gt $(date -d "11:35:00" +%s) ] && [ "$now" -lt $(date -d "12:30:00" +%s) ]; then
    Fach="BESP"
fi

if [ "$now" -gt $(date -d "12:30:00" +%s) ] && [ "$now" -lt $(date -d "13:25:00" +%s) ]; then
    Fach="BESP"
fi

if [ "$now" -gt $(date -d "13:25:00" +%s) ] && [ "$now" -lt $(date -d "14:20:00" +%s) ]; then
    Fach="MITTAGSPAUSE"
fi

if [ "$now" -gt $(date -d "14:20:00" +%s) ] && [ "$now" -lt $(date -d "15:10:00" +%s) ]; then
    Fach="DE"
fi

printf "Donnerstag %s %s\n" "$(date -d "@$now" +"%T")" "$Fach"
```
